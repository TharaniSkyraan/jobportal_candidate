(function(win, dom){
    ...
    var getThumbPosition = function(obj){
      return Math.round((obj.offsetWidth / 15) * obj.value); // Pixel
    }
    ...
    var updateProgress = function(obj, progressBgColor, progressFillColor, percentage){
      var rangeBg = 'linear-gradient(to right,  '+ progressFillColor +' 0%, '+ progressFillColor +' '+ percentage +'%, '+ progressBgColor +' '+ percentage +'%, '+ progressBgColor +' 15%)';
  ...
  win.addEventListener('resize', function () {
    fireOnceOnResize = setTimeout(update, 15);
  });
