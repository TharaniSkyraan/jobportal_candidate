...
trait FetchJobSeekers
{
    ...
    public function fetchJobSeekers($search = '', $industry_ids = array(), $functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $career_level_ids = array(), $genders = array(), $experience_ids = array(), $current_salary = 0, $expected_salary = 0, $salary_currency = '', $order_by = 'id', $limit = 10)
    {
        $asc_desc = 'DESC';
        $query = User::select($this->fields);
        $query = $this->createQuery($query, $search, $industry_ids, $functional_area_ids, $country_ids, $state_ids, $city_ids, $career_level_ids, $genders, $experience_ids, $current_salary, $expected_salary, $salary_currency);
    ...
    public function fetchIdsArray($search = '', $industry_ids = array(), $functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $career_level_ids = array(), $genders = array(), $experience_ids = array(), $current_salary = 0, $expected_salary = 0, $salary_currency = '', $field = 'users.id')
    {
    ...
    public function createQuery($query, $search = '', $industry_ids = array(), $functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $career_level_ids = array(), $genders = array(), $experience_ids = array(), $current_salary = 0, $expected_salary = 0, $salary_currency = '')
    {
        ...
        if (isset($genders[0])) {
            $query->whereIn('users.gender', $genders);
        }
        if (isset($experience_ids[0])) {
            $query->whereIn('users.experience_id', $experience_ids);
        }
    ...
    {
        $query = UserSkill::select('skill_id');
    ...
    private function getSEO($functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $career_level_ids = array(), $genders = array(), $experience_ids = array())
    {
        $description = 'Users ';
        $keywords = '';
        if (isset($functional_area_ids[0])) {
